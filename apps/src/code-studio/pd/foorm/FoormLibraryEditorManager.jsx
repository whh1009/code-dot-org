...
class FoormEditorManager extends React.Component {
  ...
  getFormattedConfigurationDropdownOptions() {
    return this.props.availableForms.map((formNameAndVersion, i) => {
      const libraryName = formNameAndVersion['name'];
      const libraryVersion = formNameAndVersion['version'];
      // maybe remove?
      ...
      return (
        <MenuItem
          key={i}
          eventKey={i}
          onClick={() => this.loadConfiguration(libraryName, libraryVersion)}
        >
  ...
  loadConfiguration(libraryName, libraryVersion) {
    ...
    $.ajax({
      url: `/api/v1/pd/foorm/library_questions`,
      type: 'get',
      data: JSON.stringify(libraryIdentifier)
    })
      .done(result => {
