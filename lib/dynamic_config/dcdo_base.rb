...
class DCDOBase
  ...
  def get(key, default)
    raise ArgumentError.new("Key is a #{key.class}, not String") unless key.is_a? String
  ...
  # @returns [DCDOBase]
  def self.create(cdo = CDO)
    env = cdo.rack_env.to_s
    ...
    if env == 'test'
      ...
      adapter = JSONFileDatastoreAdapter.new("#{cdo.dir('dashboard', cdo.dcdo_table_name)}_temp.json")
    end
