  ...
  def build_single_frame_metadatas(files)
    ...
    Parallel.map(unique_files) do |filename|
      ...
      File.open(filename) do
        s = File.read(File.join(dirname, basename + '.json'))
        obj = JSON.parse(s)
        metadata = {}
        metadata['name'] = obj['name']
        metadata['aliases'] = @options[:aliases] || []
        metadata['categories'] = @options[:categories] || []
        metadata['frameCount'] = obj['frameCount']
        metadata['frameSize'] = obj['frameSize']
        metadata['looping'] = obj['looping']
        metadata['frameDelay'] = obj['frameDelay']
